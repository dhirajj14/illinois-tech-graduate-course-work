...
read instanceID1 instanceID2 instanceID3 < <(echo $(aws ec2 run-instances --image-id $1 --instance-type t2.micro --security-group-ids $6 --key-name $9 --user-data file://install_apache.sh --count $2 --output text --query 'Instances[*].InstanceId'))
