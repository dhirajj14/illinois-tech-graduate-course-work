...
ddecade = df2.withColumn('Weather_Station', df2['value'].substr(5, 6)) \
...
.filter(year(to_date(df2['value'].substr(16,8),'yyyyMMdd')).between(2000, 2005)) \
.drop('value')

ddecade.repartition(200).write.format("csv").option("compression","lz4").mode("overwrite").save("hdfs://namenode/output/itmd-521/dpj/dpj-2000-2018-partition-two-hundred.csv")
